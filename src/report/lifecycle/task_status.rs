fn task_to_summary(task: crate::PersistedTask) -> crate::AnalysisTaskSummary {
    crate::AnalysisTaskSummary {
...
use crate::{AnalysisResult, ResultStage, TaskStatus, TaskStatusResponse};
...
impl TaskManager {
    ...
    pub async fn list_tasks(
        ...
        limit: i64,
        offset: i64,
    ) -> anyhow::Result<Vec<crate::AnalysisTaskSummary>> {
        let rows = self.analysis_store.list_tasks(limit, offset).await?;
        Ok(rows.into_iter().map(task_to_summary).collect())
    }
