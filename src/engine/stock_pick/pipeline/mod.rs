...
use crate::engine::stock_pick::{
    EnrichedCandidate, StockPickEvidencePayload, StockPickHistoryStore, parse_generated_stock_pick,
};
...
pub async fn run(
    ...
    let search_depth = helpers::normalize_stock_pick_search_depth(request.search_depth.as_deref());
    let history_retrieval = request.history_retrieval.unwrap_or(true);
    let target_output_mode =
        helpers::normalize_target_output_mode(request.target_output_mode.as_deref());
    ...
    let coarse_candidate_limit =
        helpers::derive_coarse_candidate_limit(candidate_limit, search_depth);
    let deep_candidate_limit = helpers::derive_deep_candidate_limit(pick_count, search_depth);
    let llm_review_limit = helpers::derive_llm_review_limit(pick_count, search_depth);
    ...
    let candidates =
        resolve::resolve_candidates(market_data, request, coarse_candidate_limit).await?;
    if candidates.is_empty() {
        anyhow::bail!("no stock candidates resolved for market {}", request.market);
    }

    let light_queries = if search::should_skip_light_stage_search(request, &candidates) {
        tracing::info!(
        ...
        search::build_light_search_queries(request, &candidates)
    };
    let search_time_range = helpers::stock_pick_search_time_range(search_depth);
    tracing::info!(
        market = %request.market,
        candidate_count = candidates.len(),
    ...
    for candidate in deep_pool.iter_mut() {
        let deep_queries = search::build_candidate_search_queries(candidate, request);
        let search_items = market_data
            .fetch_news_search_evidence(
                &deep_queries.iter().map(String::as_str).collect::<Vec<_>>(),
                &language,
                search_time_range,
                helpers::deep_search_limit(search_depth),
            )
            .await
            .with_context(|| {
                format!(
        ...
        let deduped_records = search::news_items_to_evidence_records(
            &candidate.symbol,
            &candidate.market,
            &candidate.theme_key,
            &deep_queries,
        ...
        candidate.news = helpers::dedupe_news_items(
            candidate
                .news
                .iter()
                .cloned()
    ...
    let prompt = build_prompt(
        helpers::market_display_label(helpers::market_kind_from_value(&request.market)),
        &strategy,
        &analysis_date,
        &language,
        &llm_selected,
...
{}",
    ...
    let picks = preselected
        ...
        .map(|item| {
            ...
            let selection_reason_codes = explanation
                ...
                .unwrap_or_else(|| helpers::default_selection_reason_codes(&item));
            let rejection_risk_flags = item.risk_snapshot.signal_codes.clone();
            let evidence_quality_score = helpers::score_evidence_quality(&item);
    ...
    for pick in picks {
        ...
        let stock_score =
            crate::engine::score::score_stock_pick(llm_client, &scoreable, &score_config).await;
        tracing::info!(
            symbol = %pick.symbol,
    ...
    // TODO: Store recommendations — previously used crate::storage::Store for PostgreSQL.
    ...
    let filtered_rejected = enriched
        ...
        .collect::<Vec<_>>();
    let constrained_rejected = selected_map
        .keys()
        .cloned()
    ...
    let history_match_summary = helpers::summarize_history_matches(&picks);
