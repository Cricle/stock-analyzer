...
mod tests {
    ...
    fn test_telemetry_state_new() {
        ...
        // Verify meter is created
        let _meter = state.meter();
    }
    ...
    #[test]
    fn test_telemetry_state_clone() {
        let state = TelemetryState::new();
        let cloned = state.clone();
        // Both should work
        let _m1 = state.meter();
        let _m2 = cloned.meter();
    }

    #[test]
    fn test_init_telemetry() {
        let telemetry = init_telemetry();
        let _meter = telemetry.meter();
    }

    #[test]
    fn test_record_analysis_task_duration() {
        let state = TelemetryState::new();
        record_analysis_task_duration(&state, "completed", "a_share", 1500.0, None);
        record_analysis_task_duration(&state, "failed", "us_equity", 500.0, Some("timeout"));
    }

    #[test]
    fn test_record_llm_usage() {
        let state = TelemetryState::new();
        record_llm_usage(&state, "gpt-4", 100, 50, 150, 2000.0, true);
        record_llm_usage(&state, "claude-3", 200, 100, 300, 3000.0, false);
    }

    #[test]
    fn test_mark_span_task() {
        // Just verify it doesn't panic
        mark_span_task("task-123", "600519", "a_share");
    }
}
