//! Unified stock analysis test example.
//!
//! Supports both Anthropic (Claude) and DeepSeek LLM providers.
//! Tests multiple markets (A股, 美股, 港股) with parallel execution.
//!
//! # Configuration
//!
//! ## Anthropic (default)
...
//! ## Optional
//! ```bash
//! export API_TIMEOUT_MS=600000  # Request timeout in ms (default: 30000 for Anthropic, 600000 for DeepSeek)
//! export ANALYSIS_DEBUG_QUICK_ONLY=1  # Quick analysis mode
//! export REPORT_KLINE_LIMIT=60  # Number of K-lines to fetch
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    let Some(llm) = setup_llm_client() else {
        eprintln!("❌ LLM config not found. Set LLM_BASE_URL + LLM_API_KEY or LLM_PROVIDER=deepseek + LLM_API_KEY");
        std::process::exit(1);
    };

    // Detect provider
    let provider = if std::env::var("LLM_PROVIDER").as_deref() == Ok("deepseek") {
        "DeepSeek"
    } else {
        "Anthropic"
    };
    ...
    let total_stocks: usize = markets.iter().map(|(_, _, s)| s.len()).sum();
    println!("=== 市场报告测试 ({} 只股票) ===", total_stocks);
