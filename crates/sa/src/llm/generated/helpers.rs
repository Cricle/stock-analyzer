...
mod tests {
    ...
    #[test]
    fn is_zero_value_number_zero() {
        assert!(is_zero_value(&json!(0)));
        assert!(is_zero_value(&json!(0.0)));
    }

    #[test]
    fn is_zero_value_string_zero() {
        assert!(is_zero_value(&json!("0")));
        assert!(is_zero_value(&json!("0.0")));
    }

    #[test]
    fn is_zero_value_non_zero() {
        assert!(!is_zero_value(&json!(1.0)));
        assert!(!is_zero_value(&json!("1")));
    }

    #[test]
    fn is_meaningful_value_null() {
        assert!(!is_meaningful_value(&json!(null)));
    }

    #[test]
    fn is_meaningful_value_empty_string() {
        assert!(!is_meaningful_value(&json!("")));
        assert!(!is_meaningful_value(&json!("  ")));
    }

    #[test]
    fn is_meaningful_value_number() {
        assert!(is_meaningful_value(&json!(42)));
    }

    #[test]
    fn is_meaningful_value_bool() {
        assert!(is_meaningful_value(&json!(true)));
    }

    #[test]
    fn is_meaningful_value_array_with_values() {
        assert!(is_meaningful_value(&json!([1, 2])));
    }

    #[test]
    fn is_meaningful_value_empty_array() {
        assert!(!is_meaningful_value(&json!([])));
    }

    #[test]
    fn meaningful_value_filters_null() {
        assert_eq!(meaningful_value(None), None);
        assert_eq!(meaningful_value(Some(json!(null))), None);
    }

    #[test]
    fn meaningful_value_keeps_valid() {
        assert_eq!(meaningful_value(Some(json!("hello"))), Some(json!("hello")));
    }
    ...
    #[test]
    fn derive_probabilities_from_text_no_signals() {
        let (up, down, sideways) = derive_probabilities_from_text("", "", "");
        // Default conservative: 0.25, 0.40, 0.35
    ...
    fn extract_numbered_trigger_lines_basic() {
        let text = "1) Check support level\n2) Verify volume\n3) Watch for breakout";
        let lines = extract_numbered_trigger_lines(text);
        assert_eq!(lines.len(), 3);
        assert_eq!(lines[0], "Check support level");
    }

    #[test]
    fn extract_numbered_trigger_lines_empty() {
        let lines = extract_numbered_trigger_lines("no numbered items here");
        assert!(lines.is_empty());
    }

    #[test]
    fn extract_object_value_from_map() {
        let value = json!({"key": "val"});
        assert_eq!(
            extract_object_value(Some(&value), &["key"]),
            Some(json!("val"))
        );
    }
    ...
    fn extract_object_value_missing_key() {
        ...
        assert_eq!(extract_object_value(Some(&value), &["key"]), None);
    }

    #[test]
    fn extract_object_value_none() {
        assert_eq!(extract_object_value(None, &["key"]), None);
    }

    #[test]
    fn split_list_like_text_semicolons() {
        let result = split_list_like_text("item1; item2; item3");
        assert_eq!(result, vec!["item1", "item2", "item3"]);
    }

    #[test]
    fn split_list_like_text_newlines() {
    ...
    fn format_price_like_text_small() {
        assert_eq!(format_price_like_text(12.34), "12.34");
    }

    #[test]
    fn extract_position_sizing_from_texts_finds_percent() {
        let result = extract_position_sizing_from_texts(&["enter 5% position"]);
        assert_eq!(result.as_deref(), Some("5%"));
    }

    #[test]
    fn extract_position_sizing_from_texts_none() {
        let result = extract_position_sizing_from_texts(&["no sizing here"]);
