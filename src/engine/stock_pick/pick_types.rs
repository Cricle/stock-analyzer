...
mod tests {
    ...
    #[test]
    fn parse_generated_stock_pick_full_response() {
        let input = json!({
            ...
            "picks": [
                {
                    ...
                    "risks": ["Supply chain"],
                    "evidence_points": ["Q3 beat"],
                    "decision_reason_codes": ["momentum"],
                    "data_gaps": []
                }
            ],
            "rejected_symbols": ["TSLA"],
            "agreement_with_system_rank": "agree",
            "override_actions": [
                {
                    "symbol": "MSFT",
                    "action": "buy",
        ...
        let response = parse_generated_stock_pick(&input.to_string()).unwrap();
        assert_eq!(response.summary, "Market looks bullish");
        assert_eq!(response.picks.len(), 1);
        assert_eq!(response.picks[0].symbol, "AAPL");
        assert_eq!(
            response.picks[0].catalysts,
            vec!["iPhone launch", "AI integration"]
        );
        assert_eq!(response.rejected_symbols, vec!["TSLA"]);
        assert_eq!(response.override_actions.len(), 1);
        assert_eq!(response.override_actions[0].action, "buy");
    }

    #[test]
    fn parse_generated_stock_pick_minimal() {
        let input = json!({
            "summary": "test",
            "picks": [],
            "rejected_symbols": [],
        ...
        let response = parse_generated_stock_pick(&input.to_string()).unwrap();
        assert_eq!(response.summary, "test");
        assert!(response.picks.is_empty());
        assert!(response.override_actions.is_empty());
    }

    #[test]
    fn parse_generated_stock_pick_missing_fields() {
        let input = json!({});
        let response = parse_generated_stock_pick(&input.to_string()).unwrap();
        assert_eq!(response.summary, "No stock picking summary returned.");
        assert!(response.picks.is_empty());
        assert_eq!(response.agreement_with_system_rank, "agree");
    }

    #[test]
    fn parse_generated_stock_pick_with_markdown_fence() {
        let input = "```json\n{\"summary\": \"test\", \"picks\": [], \"rejected_symbols\": [], \"agreement_with_system_rank\": \"agree\"}\n```";
        let response = parse_generated_stock_pick(input).unwrap();
        assert_eq!(response.summary, "test");
    }

    #[test]
    fn parse_generated_stock_pick_invalid_json() {
        let result = parse_generated_stock_pick("not json at all");
        assert!(result.is_err());
    }

    #[test]
    fn parse_generated_stock_pick_pick_defaults() {
        let input = json!({
            "summary": "test",
            "picks": [{"symbol": "X"}],
            "rejected_symbols": [],
        ...
        let response = parse_generated_stock_pick(&input.to_string()).unwrap();
        assert_eq!(response.picks[0].symbol, "X");
        assert_eq!(response.picks[0].thesis, "No thesis returned.");
        assert_eq!(response.picks[0].catalysts, vec!["No catalyst returned"]);
        assert!(response.picks[0].decision_reason_codes.is_empty());
        assert!(response.picks[0].data_gaps.is_empty());
    }

    #[test]
    fn factor_breakdown_default() {
        let fb = FactorBreakdown::default();
        assert_eq!(fb.momentum, 0.0);
        assert_eq!(fb.total, 0.0);
    }

    #[test]
    fn candidate_evidence_record_default() {
        let record = CandidateEvidenceRecord::default();
        assert!(!record.hard_negative_flag);
        assert!(record.query.is_empty());
    }

    #[test]
    fn generated_stock_pick_item_deserialize() {
        let input = json!({
            "symbol": "AAPL",
            "confidence": 0.9,
            ...
            "catalysts": ["AI"],
            "risks": ["Competition"],
            "evidence_points": ["Earnings beat"]
        });
        let item: GeneratedStockPickItem = serde_json::from_value(input).unwrap();
        assert_eq!(item.symbol, "AAPL");
        assert!(item.decision_reason_codes.is_empty());
    }
}
