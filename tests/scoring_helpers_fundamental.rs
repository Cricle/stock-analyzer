...
// --- numeric_count via numeric_tokens (NumericFieldExt is private) ---

#[test]
fn numeric_count_via_numeric_tokens() {
    assert_eq!(numeric_tokens("entry 100 stop 95").len(), 2);
    assert_eq!(numeric_tokens("no numbers").len(), 0);
}
