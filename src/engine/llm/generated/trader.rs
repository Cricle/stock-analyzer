...
use super::generated_types::GeneratedTraderDecision;
...
mod tests {
    ...
    fn make_trader(json: &str) -> GeneratedTraderDecision {
        let value: Value = serde_json::from_str(json).unwrap();
        GeneratedTraderDecision::from_value(value)
    }

    // --- rendered_proposal ---

    #[test]
    fn rendered_proposal_basic() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"good setup"}"#);
        let rendered = t.rendered_proposal();
        assert!(rendered.contains("Trader Execution Plan"));
        assert!(rendered.contains("Buy"));
    }

    #[test]
    fn rendered_proposal_with_levels() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"good","entry_price":100,"stop_loss":95,"position_sizing":"50%"}"#);
        let rendered = t.rendered_proposal();
        assert!(rendered.contains("Trade Levels"));
        assert!(rendered.contains("Entry Price"));
    ...
    fn from_value_minimal() {
        let t = make_trader(r#"{"action":"Hold","reasoning":"waiting"}"#);
        assert_eq!(t.action, "Hold");
    }

    #[test]
    fn from_value_with_entry_and_stop() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"enter","entry_price":100,"stop_loss":95}"#);
        assert!(t.entry_price.is_some());
        assert!(t.stop_loss.is_some());
    }

    // --- entry_price_number / stop_loss_number ---

    #[test]
    fn entry_price_number_value() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"r","entry_price":105.5}"#);
        assert_eq!(t.entry_price_number(), Some(105.5));
    }

    #[test]
    fn entry_price_number_string() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"r","entry_price":"$105"}"#);
        assert!(t.entry_price_number().is_some());
    }

    #[test]
    fn entry_price_number_none() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"r"}"#);
        assert_eq!(t.entry_price_number(), None);
    }

    #[test]
    fn stop_loss_number_value() {
        let t = make_trader(r#"{"action":"Buy","reasoning":"r","stop_loss":95.0}"#);
        assert_eq!(t.stop_loss_number(), Some(95.0));
    }
}
