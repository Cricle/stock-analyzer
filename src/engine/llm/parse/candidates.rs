use serde_json::Value;
use super::super::GeneratedDebateTurn;
use super::json_string::{extract_simple_json_string_field, extract_relaxed_json_string_field, extract_json_value_before_known_field};
use super::json_extract::{strip_code_fence, slice_outer_json_object, slice_first_complete_json_value, repair_common_malformed_json_variants};
use super::parse_utils::string_list_or_default;
use anyhow::bail;
pub(crate) fn parse_object_candidates_value<R>(content: &str, mapper: fn(Value) -> R) -> anyhow::Result<R> {
    let candidates = candidate_variants(content);

    let mut last_error = None;
    for candidate in candidates {
        match serde_json::from_str::<Value>(&candidate) {
            Ok(raw) => return Ok(mapper(raw)),
            Err(error) => last_error = Some(error),
        }
    }

    bail!(
        "{}",
        last_error
            .map(|error| error.to_string())
            .unwrap_or_else(|| "unknown JSON parsing error".to_string())
    )
}

pub(crate) fn parse_generated_debate_turn_lenient(content: &str) -> Option<GeneratedDebateTurn> {
    for candidate in candidate_variants(content) {
        if let Some(parsed) = parse_generated_debate_turn_lenient_candidate(&candidate) {
            return Some(parsed);
        }
    }
    None
}

pub(crate) fn parse_generated_debate_turn_lenient_candidate(content: &str) -> Option<GeneratedDebateTurn> {
    let speaker = extract_simple_json_string_field(content, "speaker")
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "Unknown".to_string());
    let stance = extract_simple_json_string_field(content, "stance")
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "neutral".to_string());
    let response = extract_relaxed_json_string_field(
        content,
        "response",
        &["confidence", "evidence_points", "risks"],
    )
    .filter(|value| !value.is_empty())
    .unwrap_or_else(|| "模型未返回辩论内容。".to_string());
    let confidence =
        extract_json_value_before_known_field(content, "confidence", &["evidence_points", "risks"])
            .unwrap_or(Value::String("unknown".to_string()));
    let evidence_points =
        extract_json_value_before_known_field(content, "evidence_points", &["risks"])
            .map(|value| string_list_or_default(Some(value), &["缺少结构化证据条目"]))
            .unwrap_or_else(|| vec!["缺少结构化证据条目".to_string()]);
    let risks = extract_json_value_before_known_field(content, "risks", &[])
        .map(|value| string_list_or_default(Some(value), &["需关注核心假设失效"]))
        .unwrap_or_else(|| vec!["需关注核心假设失效".to_string()]);

    Some(GeneratedDebateTurn {
        speaker,
        stance,
        response,
        confidence,
        evidence_points,
        risks,
    })
}

/// Replace bare control characters (U+0000..U+001F) inside JSON string
/// values with their properly escaped equivalents.  This handles LLM output
/// that contains literal newlines, tabs, or null bytes inside strings, which
/// `serde_json` rejects by default.
fn sanitize_json_control_chars(content: &str) -> String {
    let bytes = content.as_bytes();
    let mut out = Vec::with_capacity(bytes.len() + 32);
    let mut in_string = false;
    let mut escaped = false;
    let mut i = 0;
    while i < bytes.len() {
        let b = bytes[i];
        if escaped {
            out.push(b);
            escaped = false;
            i += 1;
            continue;
        }
        if in_string {
            match b {
                b'\\' => {
                    out.push(b);
                    escaped = true;
                    i += 1;
                }
                b'"' => {
                    out.push(b);
                    in_string = false;
                    i += 1;
                }
                0x00..=0x1f => {
                    match b {
                        b'\n' => out.extend_from_slice(b"\\n"),
                        b'\r' => out.extend_from_slice(b"\\r"),
                        b'\t' => out.extend_from_slice(b"\\t"),
                        0x08 => out.extend_from_slice(b"\\b"),
                        0x0c => out.extend_from_slice(b"\\f"),
                        other => {
                            let s = format!("\\u{:04x}", other); out.extend_from_slice(s.as_bytes());
                        }
                    }
                    i += 1;
                }
                _ => {
                    out.push(b);
                    i += 1;
                }
            }
        } else {
            if b == b'"' {
                in_string = true;
            }
            out.push(b);
            i += 1;
        }
    }
    String::from_utf8(out).unwrap_or_else(|_| content.to_string())
}

fn candidate_variants(content: &str) -> Vec<String> {
    let trimmed = content.trim();
    let sanitized = sanitize_json_control_chars(trimmed);
    let mut seeds = vec![sanitized, trimmed.to_string()];

    if let Some(unfenced) = strip_code_fence(trimmed) {
        seeds.push(unfenced.to_string());
    }

    if let Some(braced) = slice_outer_json_object(trimmed) {
        seeds.push(braced.to_string());
    }

    if let Some(first_json) = slice_first_complete_json_value(trimmed) {
        seeds.push(first_json.to_string());
    }

    let mut candidates = Vec::new();
    for seed in seeds {
        candidates.push(seed.clone());
        if let Some(repaired) = repair_common_malformed_json_variants(&seed)
            && repaired != seed {
                candidates.push(repaired);
            }
    }

    candidates
}

#[cfg(test)]
mod candidates_tests {
    use super::super::*;

    #[test]
    fn sanitize_json_control_chars_newline_in_string() {
        let input = r#"{"key": "line1\nline2"}"#;
        let result = sanitize_json_control_chars(input);
        assert!(result.contains("\\n"));
        // Should be valid JSON after sanitization
        assert!(serde_json::from_str::<serde_json::Value>(&result).is_ok());
    }

    #[test]
    fn sanitize_json_control_chars_tab_in_string() {
        let input = "{\"key\": \"col1\tcol2\"}";
        let result = sanitize_json_control_chars(input);
        assert!(result.contains("\\t"));
    }

    #[test]
    fn sanitize_json_control_chars_already_escaped() {
        let input = r#"{"key": "line1\\nline2"}"#;
        let result = sanitize_json_control_chars(input);
        // Should preserve the existing escape
        assert!(result.contains("\\\\n"));
    }

    #[test]
    fn sanitize_json_control_chars_outside_string() {
        let input = "{\n  \"key\": \"value\"\n}";
        let result = sanitize_json_control_chars(input);
        // Newlines outside strings should be preserved
        assert!(result.contains('\n'));
    }

    #[test]
    fn sanitize_json_control_chars_empty() {
        assert_eq!(sanitize_json_control_chars(""), "");
    }

    #[test]
    fn candidate_variants_plain_json() {
        let input = r#"{"action": "buy"}"#;
        let variants = candidate_variants(input);
        assert!(!variants.is_empty());
        assert!(variants.iter().any(|v| v.contains("buy")));
    }

    #[test]
    fn candidate_variants_with_code_fence() {
        let input = "```json\n{\"action\": \"buy\"}\n```";
        let variants = candidate_variants(input);
        assert!(variants.len() > 1);
        assert!(variants.iter().any(|v| v.contains("buy")));
    }

    #[test]
    fn candidate_variants_with_prefix_text() {
        let input = "Here is my analysis:\n{\"action\": \"sell\"}";
        let variants = candidate_variants(input);
        assert!(variants.iter().any(|v| v.contains("sell")));
    }

    #[test]
    fn sanitize_json_control_chars_carriage_return() {
        let input = "{\"key\": \"line1\rline2\"}";
        let result = sanitize_json_control_chars(input);
        assert!(result.contains("\\r"));
    }

    #[test]
    fn sanitize_json_control_chars_null_byte() {
        let input = "{\"key\": \"test\x00value\"}";
        let result = sanitize_json_control_chars(input);
        assert!(result.contains("\\u0000"));
    }
}

