...
pub(crate) fn derive_memory_reference_facts(
...
pub(crate) fn derive_setup_match_explanation(
    memory_context: &MemoryContextSnapshot,
...
mod tests {
    ...
    #[test]
    fn format_trillion() {
        assert_eq!(format_number_compact(1_500_000_000_000.0), "1.50T");
    }

    #[test]
    fn format_billion() {
        assert_eq!(format_number_compact(2_500_000_000.0), "2.50B");
    }

    #[test]
    fn format_million() {
        assert_eq!(format_number_compact(3_500_000.0), "3.50M");
    }

    #[test]
    fn format_thousand() {
        assert_eq!(format_number_compact(4_500.0), "4.50K");
    }

    #[test]
    fn format_small() {
        assert_eq!(format_number_compact(42.5), "42.50");
    }

    #[test]
    fn format_negative() {
        assert_eq!(format_number_compact(-1_500_000.0), "-1.50M");
    }

    // --- humanize_setup_tag ---

    #[test]
    fn humanize_trend_confirmed() {
        assert_eq!(humanize_setup_tag("trend_confirmed"), "趋势已确认");
    }

    #[test]
    fn humanize_fundamental_quality() {
        assert_eq!(humanize_setup_tag("fundamental_quality"), "基本面质量较强");
    }

    #[test]
    fn humanize_unknown() {
        assert_eq!(humanize_setup_tag("unknown_tag"), "");
    }

    #[test]
    fn humanize_empty() {
        assert_eq!(humanize_setup_tag(""), "");
    }

    // --- summarize_setup_tags ---

    #[test]
    fn summarize_known_tags() {
        let tags = vec!["trend_confirmed".into(), "event_driven".into()];
        let result = summarize_setup_tags(&tags);
        assert!(result.is_some());
        assert!(result.unwrap().contains("趋势已确认"));
    }

    #[test]
    fn summarize_unknown_tags() {
        let tags = vec!["unknown".into()];
        assert!(summarize_setup_tags(&tags).is_none());
    }

    #[test]
    fn summarize_empty() {
        assert!(summarize_setup_tags(&[]).is_none());
    }

    // --- derive_memory_reference_facts ---

    #[test]
    fn memory_facts_empty() {
        let result = AnalysisResult::default();
        let facts = derive_memory_reference_facts(&result);
        // Should return at least some facts from the default memory context
        // or empty if no memory context is set
        let _ = facts; // just verify it doesn't panic
    }

    // --- derive_setup_match_explanation ---

    #[test]
    fn setup_match_explanation_with_tags() {
        let mut ctx = MemoryContextSnapshot::default();
        ctx.resolved_setup_tags = vec!["trend_confirmed".into(), "event_driven".into()];
        ctx.setup_resolved_match_count = 5;
        ctx.setup_match_hit_rate = 0.6;
        let explanation = derive_setup_match_explanation(&ctx, 10);
        assert!(!explanation.details.is_empty());
    }

    #[test]
    fn setup_match_explanation_empty() {
        let ctx = MemoryContextSnapshot::default();
        let explanation = derive_setup_match_explanation(&ctx, 10);
        // May or may not have details depending on other fields
        let _ = explanation;
    }
}
