...
pub(crate) fn slice_outer_json_object(content: &str) -> Option<&str> {
    let start = content.find('{')?;
    let end = content.rfind('}')?;
    (start < end).then_some(content[start..=end].trim())
}

pub(crate) fn slice_first_complete_json_value(content: &str) -> Option<&str> {
...
pub(crate) fn repair_common_malformed_json_variants(content: &str) -> Option<String> {
