...
pub async fn run_import(
    data_dir: &str,
    request: &crate::models::QlibImportRequest,
) -> anyhow::Result<crate::models::QlibImportResponse> {
    ...
    if !request.force_reimport && symbol_limit.is_none() && existing_points > 0 {
        return Ok(crate::models::QlibImportResponse {
            started_at,
    ...
    Ok(crate::models::QlibImportResponse {
        started_at,
        finished_at: Utc::now().to_rfc3339(),
        collection: backend.collection,
        dataset_root: qlib_root.display().to_string(),
...
pub async fn run_init_from_env(
    ...
    let request = crate::models::QlibImportRequest {
