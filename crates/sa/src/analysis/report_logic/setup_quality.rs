...
pub(crate) fn normalize_gap_to_i18n_key(gap: &str) -> String {
...
mod setup_quality_tests {
    ...
    fn normalize_gap_sentiment_zh() {
        assert_eq!(normalize_gap_to_i18n_key("情绪面不佳"), "setup_gap_sentiment");
    }
    ...
    fn normalize_gap_technical() {
        ...
        assert_eq!(normalize_gap_to_i18n_key("技术面"), "setup_gap_technical_confirmation");
    }
    ...
    fn normalize_gap_earnings() {
        ...
        assert_eq!(normalize_gap_to_i18n_key("盈利下滑"), "setup_gap_earnings_data");
    }
    ...
    fn normalize_gap_insider() {
        ...
        assert_eq!(normalize_gap_to_i18n_key("内部人交易"), "setup_gap_insider_data");
    }
    ...
    fn normalize_gap_sector() {
        ...
        assert_eq!(normalize_gap_to_i18n_key("行业风险"), "setup_gap_sector_data");
    }
    ...
    fn normalize_gap_case_insensitive() {
        assert_eq!(normalize_gap_to_i18n_key("CASH FLOW missing"), "setup_gap_cash_flow");
        assert_eq!(normalize_gap_to_i18n_key("NEWS Coverage"), "setup_gap_news_coverage");
    }
    ...
    fn collect_execution_blocking_gaps_deduplicates() {
        let mut research = StructuredResearchPlan::default();
        research.missing_evidence_ladder.blocking_gaps = vec!["cash flow data".into(), "cash flow data".into()];
        let trader = StructuredTraderPlan::default();
        let portfolio = StructuredPortfolioDecision::default();
        let diagnostics = ReportDiagnostics::default();
        let gaps = collect_execution_blocking_gaps(&research, &trader, &portfolio, &diagnostics);
