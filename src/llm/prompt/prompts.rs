    ...
    pub(super) fn research_manager_prompt(
        ...
        format!(
            "You are the Research Manager and debate facilitator in the TradingAgents workflow.\n\
             ...
             NEUTRAL PRICE LEVELS: When recommending Hold (neutral view), price targets MUST be symmetrical around the current price. The upside target and downside target should be roughly equidistant from the current price. Do NOT set an ambitious upside target (e.g., analyst consensus target far above current) while using a tight stop loss near current price. For neutral views, use nearby resistance as upside target and nearby support as downside target.\n\
             ...
             NOISE FILTER RULE: When confirmation_level is within 2xATR(14) of current price, confirmation MUST require DAILY CLOSE above the level plus volume >= 1.5x 20-day average. Do NOT use intraday-only confirmation near current price.\n\
             CONFIRMATION DISTANCE RULE (MANDATORY): confirmation_level MUST be at least 2xATR(14) from current_price. This is a hard constraint, not advisory. If the nearest technical level (SMA, Bollinger, resistance) is within 2xATR of current price, you MUST NOT use it as confirmation. Instead: (1) set confirmation_level to indicate deferral, (2) in trigger_checklist, state that confirmation requires price to first pull back and then establish a new breakout level with volume. Example: if current=212, ATR=7, and Bollinger upper=214, do NOT set confirmation=214 because it is only 0.3xATR away and will trigger on noise.\n\
             HOLDING DISCIPLINE RULE: When entry and confirmation are different levels, define explicit rules: (1) hold between entry and confirmation, no adding, no early profit-taking; (2) intraday touch without close+volume is NOT confirmation; (3) close above with volume = confirmed, may add; (4) fall below entry = exit/reduce.\n\
             CONFIRMATION ≠ INVALIDATION RULE: `confirmation_level` and `invalidation_level` MUST be different prices. If they are the same, the trader cannot act — confirming the thesis simultaneously triggers the stop-loss, creating a logical deadlock. For bullish: confirmation above current price, invalidation below. For bearish: confirmation below current price, invalidation above.\n\
    ...
    pub(super) fn portfolio_decision_prompt(
        ...
        format!(
            "You are the Portfolio Manager in the TradingAgents workflow.\n\
             ...
             NOISE FILTER RULE: When `confirmation_level` is within 2xATR(14) of the current price, intraday price noise can falsely trigger it. In such cases, confirmation MUST require DAILY CLOSE above the level (not just intraday touch), plus volume >= 1.5x 20-day average. Do NOT use intraday-only confirmation when the level is within 2xATR of current price.\n\
             CONFIRMATION DISTANCE RULE (MANDATORY): confirmation_level MUST be at least 2xATR(14) from current_price. This is a hard constraint, not advisory. If the nearest technical level (SMA, Bollinger, resistance) is within 2xATR of current price, you MUST NOT use it as confirmation. Instead: (1) set confirmation_level to indicate deferral, (2) in trigger_checklist, state that confirmation requires price to first pull back and then establish a new breakout level with volume. Example: if current=212, ATR=7, and Bollinger upper=214, do NOT set confirmation=214 because it is only 0.3xATR away and will trigger on noise.\n\
             HOLDING DISCIPLINE RULE: When entry_price and confirmation_level are different levels, define explicit holding rules for the zone between them:\n\
             (1) If price moves from entry toward confirmation: hold, do NOT add, do NOT take early profit.\n\
             (2) If price touches confirmation but does NOT close above with volume: wait for next trading day's close.\n\
             (3) If price closes above confirmation with volume >= 1.5x 20d avg: confirmation triggered, may add.\n\
             (4) If price falls back below entry: exit or reduce (capital preservation).\n\
