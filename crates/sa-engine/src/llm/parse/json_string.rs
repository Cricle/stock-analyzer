fn skip_json_whitespace(content: &str, mut index: usize) -> usize {
...
fn find_json_string_end(content: &str, start: usize) -> Option<usize> {
    let bytes = content.as_bytes();
...
fn find_json_value_end(content: &str, start: usize) -> Option<usize> {
    let bytes = content.as_bytes();
...
fn decode_json_string_literal(literal: &str) -> anyhow::Result<String> {
    serde_json::from_str::<String>(literal).map_err(Into::into)
}

fn extract_simple_json_string_field(content: &str, field: &str) -> Option<String> {
...
fn extract_relaxed_json_string_field(
    content: &str,
    field: &str,
    next_fields: &[&str],
) -> Option<String> {
...
fn extract_json_value_before_known_field(
    content: &str,
    field: &str,
    next_fields: &[&str],
) -> Option<Value> {
