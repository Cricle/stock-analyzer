...
impl TaskManager {
    pub(super) async fn resolve_pending_entries(
        ...
        for entry in pending {
            ...
            let reflection = llm
                ...
                .await?;
            updates.push(crate::engine::memory::MemoryOutcomeUpdate {
                ticker: entry.ticker,
    ...
    pub(crate) async fn initial_memory_context(
        ...
        news_items: &[NewsItem],
    ) -> crate::engine::memory::MemoryContextBundleWithTags {
        let setup_tags =
            Self::derive_pre_analysis_setup_tags(ticker, market, quote, fundamentals, news_items);
        let query = crate::engine::memory::MemoryQuery {
    ...
    fn derive_pre_analysis_setup_tags(
        ...
        if let Some(quote) = quote && quote.close > 0.0 {
            let hundred = 100.0;
        ...
        if let Some(fundamentals) = fundamentals {
            ...
            let has_quality_scale = market_cap > 0.0
                && (net_income > 0.0 || free_cash_flow > 0.0 || revenue > billion);
    ...
    pub(super) fn steps_for_progress(
        ...
        report_stage_state: Option<&crate::models::ReportStageState>,
    ) -> Vec<AnalysisStep> {
        crate::TASK_STEPS
            .into_iter()
            .enumerate()
