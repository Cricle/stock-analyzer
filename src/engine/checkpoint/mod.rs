...
//!
//! Uses `crate::models::CheckpointStore` trait instead of direct Redis access.
...
impl TaskCheckpointStore {
    pub fn new(inner: Arc<dyn crate::models::CheckpointStore>) -> Self {
        Self {
            inner,
    ...
    pub async fn save(&self, checkpoint: &TaskCheckpoint) -> anyhow::Result<()> {
        ...
        let analysis_checkpoint = crate::models::StoredCheckpoint {
...
mod tests {
    ...
    #[test]
    fn test_hex_16_basic() {
        let bytes = [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef];
        let result = hex_16(&bytes);
        assert_eq!(result, "0123456789abcdef");
    }

    #[test]
    fn test_hex_16_zeros() {
        let bytes = [0u8; 8];
        let result = hex_16(&bytes);
    ...
    #[test]
    fn test_hex_16_truncates_long_input() {
        let bytes = [0xff; 32];
        let result = hex_16(&bytes);
        assert_eq!(result.len(), 16);
        assert_eq!(result, "ffffffffffffffff");
    }

    #[test]
    fn test_hex_16_short_input() {
        let bytes = [0xab, 0xcd];
        let result = hex_16(&bytes);
    ...
    fn test_thread_id_deterministic() {
        let id1 = TaskCheckpointStore::thread_id("task-1", "600519", "2024-01-15");
        let id2 = TaskCheckpointStore::thread_id("task-1", "600519", "2024-01-15");
        assert_eq!(id1, id2);
    }

    #[test]
    fn test_thread_id_different_inputs() {
        let id1 = TaskCheckpointStore::thread_id("task-1", "600519", "2024-01-15");
        let id2 = TaskCheckpointStore::thread_id("task-2", "600519", "2024-01-15");
        assert_ne!(id1, id2);
    }

    #[test]
    fn test_thread_id_case_insensitive_symbol() {
        let id1 = TaskCheckpointStore::thread_id("task-1", "600519", "2024-01-15");
        let id2 = TaskCheckpointStore::thread_id("task-1", "600519.SH", "2024-01-15");
        assert_ne!(id1, id2);
    }

    #[test]
    fn test_thread_id_length() {
        let id = TaskCheckpointStore::thread_id("task-1", "AAPL", "2024-01-15");
        assert_eq!(id.len(), 16);
    }

    #[test]
    fn test_task_checkpoint_serialization() {
        let checkpoint = TaskCheckpoint {
            task_id: "task-123".to_string(),
            symbol: "600519".to_string(),
            analysis_date: "2024-01-15".to_string(),
            stage: "research".to_string(),
            node: "fundamentals".to_string(),
            result: AnalysisResult {
                task_id: "task-123".to_string(),
                report_id: "report-1".to_string(),
                symbol: "600519".to_string(),
                stock_name: "Kweichow Moutai".to_string(),
                analysis_date: "2024-01-15".to_string(),
                market_type: "a_share".to_string(),
        ...
        let deserialized: TaskCheckpoint = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.task_id, "task-123");
        assert_eq!(deserialized.step, 5);
    }

    #[test]
    fn test_checkpoint_write_serialization() {
        ...
        let deserialized: CheckpointWrite = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.stage, "research");
