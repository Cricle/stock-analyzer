...
mod tests {
    ...
    use super::{ChatMessageResponse, extract_stream_delta_text, is_retryable_llm_error};
    ...
    fn extract_stream_delta_text_from_array_content() {
        let value = json!({
            "choices": [{"delta": {"content": [{"text": "part1"}, {"text": "part2"}]}}]
        });
    ...
    fn extract_stream_delta_text_from_array_with_content_field() {
        let value = json!({
            "choices": [{"delta": {"content": [{"content": "data"}]}}]
        });
    ...
    fn extract_stream_delta_text_falls_back_to_message_array() {
        let value = json!({
            "choices": [{"message": {"content": [{"text": "msg"}]}}]
        });
    ...
    fn extract_stream_delta_text_empty_delta_string() {
        ...
        // Empty string delta should fall through to message, which is absent
        assert_eq!(extract_stream_delta_text(&value), "");
    }
    ...
    fn extract_stream_delta_text_skips_empty_array_items() {
        let value = json!({
            "choices": [{"delta": {"content": [{"text": ""}, {"text": "ok"}]}}]
        });
