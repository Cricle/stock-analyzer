impl LlmClient {
    ...
    pub(crate) fn research_manager_prompt(
    ...
    pub(crate) fn portfolio_decision_prompt(
...
mod prompts_tests {
    ...
    #[test]
    fn bounded_text_empty() {
        assert_eq!(LlmClient::bounded_text("", 100), "");
    }

    #[test]
    fn bounded_text_zero_max() {
        assert_eq!(LlmClient::bounded_text("hello", 0), "");
    }

    #[test]
    fn bounded_text_short_text() {
        assert_eq!(LlmClient::bounded_text("hello", 100), "hello");
    }

    #[test]
    fn bounded_text_exact_max() {
        let text = "a".repeat(100);
        assert_eq!(LlmClient::bounded_text(&text, 100), text);
    }

    #[test]
    fn bounded_text_truncates_long() {
        let text = "a".repeat(200);
        let result = LlmClient::bounded_text(&text, 100);
        assert!(result.chars().count() <= 100);
        assert!(result.contains("truncated"));
    }

    #[test]
    fn bounded_text_preserves_head_and_tail() {
        let text = format!("{}middle{}", "HEAD".to_string() + &"x".repeat(50), "TAIL".to_string() + &"y".repeat(50));
        let result = LlmClient::bounded_text(&text, 80);
        assert!(result.contains("HEAD"));
    }

    #[test]
    fn compact_fact_sheet_limits_to_800() {
        let text = "a".repeat(1000);
        let result = LlmClient::compact_fact_sheet(&text);
        assert!(result.chars().count() <= 800);
    }

    #[test]
    fn compact_past_context_limits_to_600() {
        let text = "a".repeat(800);
        let result = LlmClient::compact_past_context(&text);
        assert!(result.chars().count() <= 600);
    }

    #[test]
    fn append_retry_hint_none() {
        let result = LlmClient::append_retry_hint("prompt", None);
        assert_eq!(result, "prompt");
    }

    #[test]
    fn append_retry_hint_empty() {
        let result = LlmClient::append_retry_hint("prompt", Some("  "));
        assert_eq!(result, "prompt");
    }

    #[test]
    fn append_retry_hint_some() {
        let result = LlmClient::append_retry_hint("prompt", Some("retry this"));
        assert_eq!(result, "prompt\n\nretry this");
    }
}
