...
use super::stats::VectorMemoryPayload;
use super::{
...
impl TradingMemoryLog {
    pub(super) fn vector_point_id(entry_id: &str) -> String {
    ...
    pub(super) async fn vector_upsert_entry(&self, entry: &MemoryEntry) -> anyhow::Result<()> {
        ...
        let point_id = Self::vector_point_id(&Self::entry_id(&entry.ticker, &entry.trade_date));
    ...
    pub(super) async fn vector_upsert_research_record(
        &self,
        ticker: &str,
        trade_date: &str,
        rating: &str,
        ...
        let point_id = Self::vector_point_id(&Self::research_entry_id(ticker, trade_date));
        let payload = serde_json::json!({
    ...
    pub(super) async fn vector_past_context_bundle(
        &self,
        query: &MemoryQuery,
        same_limit: usize,
        cross_limit: usize,
    ...
    pub(super) async fn vector_search_filtered(
        ...
        for hit in hits {
            ...
            let payload: VectorMemoryPayload = match serde_json::from_value(hit.payload) {
                Ok(p) => p,
                Err(_) => continue,
            };
    ...
    pub(super) fn payload_to_entry(payload: VectorMemoryPayload) -> MemoryEntry {
    ...
    pub async fn vector_upsert_qa(&self, qa: &super::QaMemoryEntry) -> anyhow::Result<()> {
        let Some(store) = self.vector_store.as_deref() else {
            return Ok(());
        };
        if !self.embedding.retrieval_enabled {
        ...
        let point_id = format!(
            "qa:{}:{}",
            qa.task_id,
            Self::vector_point_id(&qa.question_text)
        );
        let payload = serde_json::json!({
            "entry_kind": "qa",
            "qa_type": qa.qa_type,
