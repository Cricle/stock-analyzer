fn derive_evidence_cards(references: &ReportReferenceSnapshot) -> Vec<ReportEvidenceCard> {
...
pub(crate) fn derive_news_insights(
...
fn news_watch_next_summary(decision: &DecisionView) -> LocalText {
...
fn has_report_diagnostic(items: &[ReportDiagnosticItem], code: &str) -> bool {
    items.iter().any(|item| item.code == code)
}

#[cfg(test)]
mod news_insights_tests {
    use super::super::*;
    ...
    fn derive_evidence_cards_direction_mapping() {
        let refs = ReportReferenceSnapshot {
            market: vec![
                ReferenceFactItem { key: "k1".into(), value: "v1".into(), emphasis: "success".into(), ..Default::default() },
                ReferenceFactItem { key: "k2".into(), value: "v2".into(), emphasis: "warning".into(), ..Default::default() },
                ReferenceFactItem { key: "k3".into(), value: "v3".into(), emphasis: "other".into(), ..Default::default() },
            ],
            ..Default::default()
        };
        let cards = derive_evidence_cards(&refs);
