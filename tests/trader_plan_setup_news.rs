use sa::{
    AnalysisResult, ConfidenceCap, ConfidenceProfile, DecisionAction, DecisionConfidenceBand,
    DecisionView, DecisionViewDirection, HistoricalMemoryHighlight, LocalText,
    MemoryContextSnapshot, NewsInsight, PriceContext, Rating, ReferenceFactItem,
    ReportDiagnosticItem, ReportDiagnostics, ReportReferenceSnapshot, StructuredPortfolioDecision,
    StructuredReflection, StructuredResearchPlan, StructuredRiskAssessment, StructuredTraderPlan,
    TechnicalIndicatorConclusion, TechnicalValues, derive_action_guides,
    derive_memory_reference_facts, derive_news_insights, derive_setup_match_explanation,
    derive_technical_conclusions, detect_disclosure_sequence_complexity, is_semantically_similar,
};

fn base_analysis_result(task_id: &str, symbol: &str, market: &str) -> AnalysisResult {
    AnalysisResult {
        task_id: task_id.to_string(),
        report_id: format!("report-{task_id}"),
...
fn watcher_guide_calls_out_unverified_setup_history() {
    let result = base_analysis_result("watcher", "603629", "CN");
...
fn disclosure_sequence_complexity_detects_clustered_capital_markets_filings() {
    let result = base_analysis_result("disclosure", "TEST", "US");
