...
fn e2e_data_fetch_diagnosis() {
    let diagnosis = stock_analyzer::DataFetchDiagnosis::new("quote", "AAPL");
...
fn e2e_news_date_normalization() {
    // Valid date
    let result = stock_analyzer::normalized_news_date("2026-06-21");
    assert!(result.is_some());
    assert_eq!(result.unwrap(), "2026-06-21");

    // Invalid date
    let result = stock_analyzer::normalized_news_date("not a date");
