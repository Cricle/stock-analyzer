...
impl crate::TaskManager {
    fn research_manager_needs_deep_llm(result: &AnalysisResult, params: &TaskRunParams) -> bool {
        if crate::config::analysis_debug_quick_only() {
            return false;
        }
    ...
    pub(crate) async fn run_bear_researcher_node(
        ...
        {
            crate::analysis::graph::push_checkpoint(
                result,
                "investment_debate",
                "Bull/Bear Debate",
                "completed",
...
impl crate::TaskManager {
    ...
    pub(crate) async fn run_research_manager_stage(
        ...
        if result.agent_state.investment_plan.trim().is_empty() {
            ...
            result.agent_state.structured_research_plan = sa_models::StructuredResearchPlan {
                ...
                missing_evidence_ladder: sa_models::MissingEvidenceLadder {
    ...
    pub(crate) async fn run_trader_stage(
        ...
        if result.agent_state.trader_investment_plan.trim().is_empty() {
            ...
            result.agent_state.structured_trader_plan = sa_models::StructuredTraderPlan {
                ...
                proposal: sa_models::LocalText::new(trader.action.trim().to_string()),
    ...
    pub(crate) async fn run_portfolio_stage(
        ...
        if result.agent_state.final_trade_decision.trim().is_empty() {
            ...
            result.agent_state.structured_portfolio_decision =
                sa_models::StructuredPortfolioDecision {
                    rating: sa_models::Rating::parse(&portfolio_decision.rating),
