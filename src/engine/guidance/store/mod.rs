...
mod store_tests {
    ...
    fn market_ttl_a_share_market_hours() {
        // A-share market hours: 09:15-07:00 UTC (75-420 minutes of day in UTC+8)
        // This test uses the current time, so we just verify the function doesn't panic
        // and returns a valid duration
    ...
    fn market_ttl_unknown_market() {
        ...
        // Unknown market is never in hours, so always 12x
...
impl crate::models::CacheStore for NoopCacheStore {
    ...
    async fn list_entries(&self, _prefix: &str) -> anyhow::Result<Vec<crate::models::CacheEntry>> {
        Ok(vec![])
    }
}
...
impl crate::models::VectorStore for NoopVectorStore {
    ...
    async fn search(
        ...
        _top_k: usize,
    ) -> anyhow::Result<Vec<crate::models::VectorSearchHit>> {
        Ok(vec![])
    }
