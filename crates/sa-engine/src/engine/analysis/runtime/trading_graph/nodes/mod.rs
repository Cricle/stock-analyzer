    ...
    move |ctx| {
        ...
        Box::pin(async move {
            ...
            let decision = {
                ...
                crate::engine::llm::retry::retry_with_diagnosis(
                    ...
                    |retry_hint: Option<&str>| {
                        ...
                        async move {
                            ...
                            llm.generate_analyst_decision(crate::engine::llm::AnalystDecisionParams {
                                symbol: &symbol,
                    ...
                    |pair| crate::engine::llm::parse::validate_analyst_decision(&pair.0, &pair.1),
                    crate::engine::llm::retry::default_retry_hint_builder,
...
async fn apply_analyst_report(
    ...
    params: &TaskRunParams,
    analyst_key: &str,
    report: crate::engine::llm::GeneratedRoleReport,
) -> anyhow::Result<()> {
    tracing::info!(
        task_id = %result.task_id,
        symbol = %result.symbol,
