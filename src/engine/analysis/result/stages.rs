...
impl crate::TaskManager {
    ...
    pub(crate) async fn run_bull_researcher_node(
        ...
        turns.push(crate::engine::analysis::graph::debate_turn_from_generated(&bull_turn));
    ...
    pub(crate) async fn run_bear_researcher_node(
        ...
        let mut turns = debate_turns;
        turns.push(crate::engine::analysis::graph::debate_turn_from_generated(&bear_turn));
        result.graph.investment_debate = InvestmentDebateState {
            bull_history: bull_history.trim().to_string(),
            bear_history: bear_history.trim().to_string(),
            history: format!(
...
impl crate::TaskManager {
    ...
    pub(crate) async fn run_risk_round(
        ...
        turns.push(crate::engine::analysis::graph::debate_turn_from_generated(&aggressive));
        turns.push(crate::engine::analysis::graph::debate_turn_from_generated(&conservative));
        turns.push(crate::engine::analysis::graph::debate_turn_from_generated(&neutral));
        ...
        if result.agent_state.investment_plan.trim().is_empty() {
            ...
            for retry in 0..=2u32 {
                let hint = if retry == 0 {
                    ...
                    Some(crate::engine::llm::retry::default_retry_hint_builder(&last_issues, retry))
                };
                let candidate = research_llm.generate_research_manager(crate::engine::llm::ResearchManagerParams {
                    symbol: &result.symbol,
                    market_type: &params.market_type,
                    analysis_date: &params.analysis_date,
    ...
    pub(crate) async fn run_trader_stage(
        ...
        if result.agent_state.trader_investment_plan.trim().is_empty() {
            ...
            for retry in 0..=2u32 {
                ...
                let candidate = quick_llm.generate_trader_decision(crate::engine::llm::TraderDecisionParams {
                    symbol: &result.symbol,
                    market_type: &params.market_type,
                    analysis_date: &params.analysis_date,
    ...
    pub(crate) async fn run_portfolio_stage(
        ...
        if result.agent_state.final_trade_decision.trim().is_empty() {
            ...
            for retry in 0..=2u32 {
                ...
                let candidate = deep_llm.generate_portfolio_decision(crate::engine::llm::PortfolioDecisionParams {
                    symbol: &result.symbol,
                    market_type: &params.market_type,
                    analysis_date: &params.analysis_date,
