...
mod tests {
    ...
    // --- raw_directional_anchor ---

    #[test]
    fn raw_anchor_empty() {
        let portfolio = StructuredPortfolioDecision::default();
        assert_eq!(portfolio.raw_directional_anchor(), None);
    }

    #[test]
    fn raw_anchor_present() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.raw_rating = "Buy".to_string();
        assert_eq!(portfolio.raw_directional_anchor(), Some("Buy"));
    }
    ...
    fn override_no_raw() {
        ...
        assert!(!portfolio.has_authoritative_override(&trader));
    }

    #[test]
    fn override_rating_changed() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.raw_rating = "Buy".to_string();
        portfolio.rating = Rating::Hold;
        let trader = StructuredTraderPlan::default();
        assert!(portfolio.has_authoritative_override(&trader));
    }

    #[test]
    fn override_action_changed() {
        let portfolio = StructuredPortfolioDecision::default();
        let mut trader = StructuredTraderPlan::default();
        trader.raw_action = "Buy".to_string();
        trader.action = "Hold".to_string();
        assert!(portfolio.has_authoritative_override(&trader));
    }

    #[test]
    fn override_no_change() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.raw_rating = "Buy".to_string();
        portfolio.rating = Rating::Buy;
        let mut trader = StructuredTraderPlan::default();
        trader.raw_action = "Hold".to_string();
        trader.action = "Hold".to_string();
        assert!(!portfolio.has_authoritative_override(&trader));
    }
    ...
    fn render_markdown_basic() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.confidence = "75".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        portfolio.executive_summary = "看涨".to_string();
        portfolio.investment_thesis = "基本面强劲".to_string();
        let md = portfolio.render_markdown();
        assert!(md.contains("Portfolio Manager Decision"));
        assert!(md.contains("Buy"));
        assert!(md.contains("75"));
    }

    #[test]
    fn render_markdown_with_levels() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.confidence = "75".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        ...
        portfolio.invalidation_level = "95".to_string();
        portfolio.price_target = "120".to_string();
        portfolio.time_horizon = "3个月".to_string();
        let md = portfolio.render_markdown();
        assert!(md.contains("Confirmation Level"));
        assert!(md.contains("Invalidation Level"));
        assert!(md.contains("Price Target"));
        assert!(md.contains("Time Horizon"));
    }

    #[test]
    fn render_markdown_with_rationale() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.confidence = "75".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        portfolio.executive_summary = "看涨".to_string();
        portfolio.investment_thesis = "基本面强劲".to_string();
        portfolio.rationale = "强烈看涨理由".to_string();
        let md = portfolio.render_markdown();
        assert!(md.contains("Why This Call Won"));
    }

    #[test]
    fn render_markdown_with_target_reference() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.confidence = "75".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        portfolio.executive_summary = "看涨".to_string();
        portfolio.investment_thesis = "基本面强劲".to_string();
        portfolio.target_reference = "120.0".to_string();
        portfolio.target_condition = "突破115后".to_string();
        let md = portfolio.render_markdown();
        assert!(md.contains("Target Reference"));
        assert!(md.contains("Target Condition"));
    }

    // --- authoritative_summary ---

    #[test]
    fn authoritative_summary_basic() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.executive_summary = "看涨摘要".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        let trader = StructuredTraderPlan::default();
        let dv = DecisionView::default();
        let summary = portfolio.authoritative_summary(&trader, 70, &CoreResearchCall::LeanBuy, &dv);
        assert!(!summary.is_empty());
        assert!(summary.contains("Buy"));
    }

    #[test]
    fn authoritative_summary_with_confirmation() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.executive_summary = "看涨摘要".to_string();
        portfolio.risk_assessment = "低风险".to_string();
        ...
        let trader = StructuredTraderPlan::default();
        let dv = DecisionView::default();
        let summary = portfolio.authoritative_summary(&trader, 70, &CoreResearchCall::BuyOnConfirmation, &dv);
        assert!(!summary.is_empty());
    }
    ...
    #[test]
    fn authoritative_thesis_basic() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        portfolio.investment_thesis = "基本面强劲".to_string();
        let trader = StructuredTraderPlan::default();
        let thesis = portfolio.authoritative_investment_thesis(&trader, 70);
        assert!(!thesis.is_empty());
        assert!(thesis.contains("Buy"));
    }

    // --- authoritative_rationale ---

    #[test]
    fn authoritative_rationale_basic() {
        let mut portfolio = StructuredPortfolioDecision::default();
        portfolio.rating = Rating::Buy;
        let trader = StructuredTraderPlan::default();
        let rationale = portfolio.authoritative_rationale(&trader, 70, "校准原因");
        assert!(!rationale.is_empty());
        assert!(rationale.contains("Buy"));
        assert!(rationale.contains("校准原因"));
    }
}
