...
use crate::models::{AnalysisResult, TaskEvent, TaskStatus};
use crate::{TaskManager, TaskRunParams};

use super::formatters::build_technical_summary;

impl TaskManager {
    pub(in crate::engine::analysis::lifecycle) async fn run_task(
        &self,
        task_id: String,
        ...
        let mut result = crate::engine::analysis::runtime::TradingAgentsGraph::prepare_result(
            self, &task, &params,
        )
        .await?;
        tracing::info!(
        ...
        params.memory_context =
            crate::engine::task_manager::memory_snapshot_from_bundle(&refined_memory_context);
        ...
        if !consistency_issues.is_empty() {
            ...
            result.artifacts.diagnosis_summary = Some(
                crate::models::DiagnosisSummary::from_issues(&consistency_issues),
            );
        }
    ...
    pub(in crate::engine::analysis::lifecycle) async fn publish_failure(
        &self,
        task_id: &str,
