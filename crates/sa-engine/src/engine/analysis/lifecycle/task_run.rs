...
struct CoreMarketData {
    ...
    news_items: Vec<crate::data::NewsItem>,
    market_chart: crate::models::ReportMarketChart,
...
fn format_earnings_forecast_summary(
    ...
    match result {
        Ok(Ok(items)) if !items.is_empty() => {
            let matching: Vec<&crate::data::EarningsForecast> = items.iter()
...
fn build_technical_summary(chart: &crate::models::ReportMarketChart) -> String {
...
impl TaskManager {
    ...
    pub async fn task_run_params_from_request(
        ...
        TaskRunParams {
            ...
            memory_context: crate::engine::task_manager::memory_snapshot_from_bundle(&memory_context),
...
impl TaskManager {
    ...
    async fn fetch_core_market_data(
        ...
        let (quote, quote_diagnosis) = match _quote_result {
            ...
            Err(error) => {
                ...
                (None, crate::data::diagnosis::DataFetchDiagnosis::new("quote", &task.symbol))
            }
        };
        ...
        let (candles_data, candles_diagnosis) = match _candles_result {
            ...
            Err(error) => {
                ...
                (None, crate::data::diagnosis::DataFetchDiagnosis::new("candles", &task.symbol))
            }
        };
        ...
        let market_chart = match candles_data {
            Some(items) => {
                ...
                crate::models::ReportMarketChart {
            ...
            None => {
                ...
                crate::models::ReportMarketChart::default()
            }
        };
    ...
    pub(super) async fn run_task(
        ...
        params.memory_context = crate::engine::task_manager::memory_snapshot_from_bundle(&refined_memory_context);
        ...
        let consistency_issues =
            crate::engine::analysis::diagnosis::ConsistencyValidator::validate_and_fix(&mut result);
        if !consistency_issues.is_empty() {
            ...
            result.artifacts.diagnosis_summary =
                Some(crate::models::DiagnosisSummary::from_issues(&consistency_issues));
        }
